...
// upload.array('images', 3); [if we had an array with the same name]
...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  ...
  // Resize other images
  req.body.images = [];
  let imagesBuffersArr = [];
  req.files.images.forEach((image, i) => {
    req.body.images.push(`tour-${req.params.id}-${Date.now()}-${i + 1}.jpeg`);
    imagesBuffersArr.push(image.buffer);
  });
  imagesBuffersArr.forEach(async (buffer, i) => {
    await sharp(buffer)
      .resize(2000, 1333)
      .toFormat('jpeg')
      .jpeg({ quality: 90 })
